...
# %% Functions
def fit_module(params, ind_vars, target="time", size=20, cols=None, name="", layers=None, **mod_kwargs):
    ...
    **mod_kwargs,
    ...
    for mat in mats:
        for label in materials.database.loc[
            ...
            if cols is None:
                res_temp = fit_module(
                    params, ind_vars, target, size, name=f"_{mat}_{label}", layers=layers, **mod_kwargs
                )
                res = {**res, **res_temp}
            else:
...
class MatDatabase(object):
    ...
    # def __init__(self):
    path = "C:\\Users\\j2cle\\Work Docs\\Data\\Databases"
    file = "material_data.xlsx"
    database = pd.read_excel(f"{path}\\{file}", index_col=[0, 1, 2])
...
path = "C:\\Users\\j2cle\\Work Docs\\Data\\Analysis\\Simulations\\Module"
...
ut.save(
    btt,
    f"{path}\\{stacks[0]}\\Soda_Varied_low",
    "temp_e_time4",
)
...
# ut.save(
#     btt_boro,
#     f"{path}\\{stacks[0]}\\Boro_Varied_low",
#     "temp_e_time3",
# )
...
# ut.save(
#     tri_time_v5,
#     f"{path}\\{stacks[0]}\\Varied_resistivities",
#     "all_time_v5",
# )
