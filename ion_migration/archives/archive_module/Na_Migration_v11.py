...
class Layer(object):
    ...
    def __init__(
        ...
        self.btt_target = btt_target

        self.time = up.Time(1)

        # self.lay_np = NernstPlanck(self.np_info)
        self.lay_np = self.np_solver(btt_target)
    ...
    def thick(self):
        ...
        if "thick" in self.btt_target:
            # if not hasattr(self, "lay_np"):
            #     self.lay_np = self.np_solver("thick")
            return up.Length(self.lay_np, "cm")
        else:
    ...
    def temp(self):
        """Return thickness of layer."""
        if "temp" in self.btt_target:
            # if not hasattr(self, "lay_np"):
            #     self.lay_np = self.np_solver("temp")
            return up.Temp(self.lay_np, "C")
        else:
            return self._temp
    ...
    def efield(self):
        ...
        if "efield" in self.btt_target:
            # if not hasattr(self, "lay_np"):
            #     self.lay_np = self.np_solver("efield")
            return up.Volt(self.lay_np, "V")
        else:
            return self._efield
    ...
    def resistance(self, val):
        ...
        if val is not None:
            if isinstance(val, up.BaseUnits):
                ...
                self.resistivity = up.Res(*val).Ohm * self.area.cm / self.thick.cm
            else:
    ...
    def depth(self):
        """Return thickness of layer."""
        if "depth" in self.btt_target:
            # if not hasattr(self, "lay_np"):
            #     self.lay_np = self.np_solver("depth")
            return up.Length(self.lay_np, "cm")
        else:
    ...
    def time(self):
        """Return sum of squared errors (pred vs actual)."""
        if "time" in self.btt_target:
            # if not hasattr(self, "lay_np"):
            #     self.lay_np = self.np_solver("time")
            return up.Time(self.lay_np, "s")
        else:
    ...
    def dif(self):
        """Return sum of squared errors (pred vs actual)."""
        if "dif" in self.btt_target:
            # if not hasattr(self, "lay_np"):
            #     self.lay_np = self.np_solver("dif")
            return self.lay_np
        else:
    ...
    def np_solver(
        ...
        """Return sum of squared errors (pred vs actual)."""
        if target in self.np_info.columns and (self.data.loc["dif","var1"] != 0 or "dif" in self.btt_target):
...
#     ["soda", {"resis": 2, "thick": "metric"}],
#     ["eva", {"resis": 1, "dif": "max", "thick": "common"}],
#     ["sinx", {"resis": 4, "dif": 3, "thick": [80,"mm"]}],
#     ["si", {"thick": 4.05}],
# ]
...
# slg_params = [
#     ["soda", {"resis": 1, "thick": "metric"}],
#     ["eva", {"resis": 4, "dif": "max", "thick": "common"}],
#     ["sinx", {"resis": 4, "dif": 1}],
# ]

# temp_sys_v = Module(slg_vidr_params, 1500, 50, [temp_var, sys_volt_var], temp=85, sourceC=1e19, conc=1e16, btt_solver='np')
# btt1 = temp_sys_v.result(layer=1, as_df=True)

# resistance_efield = Module(slg_vidr_params, 1500, 10, [res_e_var, efield_var], temp=85, sourceC=1e19, conc=1e16, btt_solver='np')
# btt2 = resistance_efield.result(layer=1, as_df=True)

# resistivity_efield = Module(slg_vidr_params, 1500, 50, [resist_e_var, efield_var], temp=85, sourceC=1e19, conc=1e16, btt_solver='np')
# btt3 = resistivity_efield.result(layer=1, as_df=True)
# al_single_stack = VoltDev(al_eva_params, 1000, temp=25, area=4**2, sourceC=1e19, conc=1e16, btt_solver='np')
# glass_stack = VoltDev(glass_eva_params, 1500, temp=85, area=4**2, sourceC=1e19, conc=1e16, btt_solver='np')
# module1_stack = VoltDev(slg_params, 1500, temp=85, area=4**2, sourceC=1e19, conc=1e16, btt_solver='np')
#%% Simulations
...
# dep_var1 = [
#     {"param": "dif", "layer": 1, "unit": None, "calc": None},
# ]


# rhoe_rhog = Module(slg_vidr_params, 1500, 10, [resist_e_var, resist_g_var], dep_var = dep_var1, temp=85, sourceC=1e19, conc=1e16, btt_solver='np', btt_target='btt')
# rhoe_rhog.stack[1].time = up.Time(4,"d").s
# rhoe_rhog.stack[1].btt = up.Time(4,"d").s
# rhoe_rhog.solver()

# rhoe_rhog = Module(slg_vidr_params, 1500, 25, [resist_e_var, resist_g_var], temp=85, sourceC=1e19, conc=1e16, btt_solver='np')
# btt1 = rhoe_rhog.result(layer=1, as_df=True)
# e11 = btt1['efield']
# b11 = btt1['btt']
# btt2 = rhoe_rhog.result(layer=1, as_df=True)
# e12 = btt2['efield']
# b12 = btt2['btt']
# btt3 = rhoe_rhog.result(layer=2, as_df=True)
# e13 = btt3['efield']
# b13 = btt3['btt']

# rhoe_rhon = Module(slg_vidr_params, 1500, 10, [temp_var, efield_var], temp=85, sourceC=1e19, conc=1e16, btt_solver='np')
# btt2 = rhoe_rhon.result(layer=1, as_df=True)
# e21 = btt1['efield']
# b21 = btt1['btt']
# btt2 = rhoe_rhon.result(layer=1, as_df=True)
# e22 = btt2['efield']
# b22 = btt2['btt']
# btt3 = rhoe_rhon.result(layer=2, as_df=True)
# e23 = btt3['efield']
# b23 = btt3['btt']

# rhon_rhog = Module(slg_vidr_params, 1500, 25, [resist_n_var, resist_g_var], temp=85, sourceC=1e19, conc=1e16, btt_solver='np')
# btt1 = rhon_rhog.result(layer=0, as_df=True)
# e31 = btt1['efield']
# b31 = btt1['btt']
# btt2 = rhon_rhog.result(layer=1, as_df=True)
# e32 = btt2['efield']
# b32 = btt2['btt']
# btt3 = rhon_rhog.result(layer=2, as_df=True)
# e33 = btt3['efield']
# b33 = btt3['btt']
...
# rhoe_difn_v = Module(slg_params, 1500, 25, [resist_n_var, dif_n_var], temp=85, sourceC=1e19, conc=1e16, btt_solver='np')
