...
Busted Functions:
    eqn_sets:
        NP Class char_eq find  x0
    Char_time & np_cost:
        optemize function for np_sim
...
class NernstPlanck(object):
    ...
    def char_eq(self, target="time", as_set=False):
        ...
        param_values["boltz"] = get_const("boltzmann", ["eV", "K"])
...
class Layer(object):
    ...
    def _thick(self):
        ...
        if self.data.var2["thick"] != 0:
            var = arrh(temp=self._temp.K, *self.data.loc["thick", ["var1", "var2"]])
        else:
            var = self.data.var1["thick"]
        return convert_val(var, self.data.unit["thick"], "cm")
    ...
    def resistivity(self, val):
        ...
        if val is not None:
            ...
            if self.rt_link and self.data.var2["resis"] != 0:
                self.temp = Temp(
                    arrh(
                        result=self._resistivity,
                        *self.data.loc["resis", ["var1", "var2"]],
                    ),
                    "K",
                    1,
                    "C",
    ...
    def _resistivity(self):
        """rho = rho0 * exp(Ea / k_B T)."""
        return arrh(temp=self._temp.K, *self.data.loc["resis", ["var1", "var2"]])
    ...
    def _er(self):
        ...
        if self.data.var2["perm"] != 0:
            return arrh(temp=self._temp.K, *self.data.loc["perm", ["var1", "var2"]])
        else:
    ...
    def dif(self, val):
        if val is not None:
            ...
            if self.dt_link and self.data.var2["dif"] != 0:
                self.temp = Temp(
                    arrh(result=self._dif, *self.data.loc["resis", ["var1", "var2"]]),
                    "K",
                    1,
                    "C",
                )
    ...
    def _dif(self):
        ...
        if self.data.var2["dif"] != 0:
            return arrh(temp=self._temp.K, *self.data.loc["dif", ["var1", "var2"]])
        else:
    ...
    def depth(self, val):
        if val is not None:
            self._depth = self.parse_unit(Length, val, "cm")
    ...
    def time(self, val):
        """Return sum of squared errors (pred vs actual)."""
        if val is not None:
            self._time = self.parse_unit(Time, val, "s")
    ...
    def temp(self, val):
        if val is not None:
            self._temp = self.parse_unit(Temp, val, "C")
            if float(str(self._temp)) >= self.temp_threshold:
                self._temp = self.parse_unit(Temp, val, "K", 1, "C")
    ...
    def efield(self, val):
        ...
        if val is not None:
            self._efield = self.parse_unit(Volt, val, "V")
    ...
    def area(self, val):
        if val is not None:
            self._area = self.parse_unit(Length, val, "cm", 2)
            if self._area.exp != 2:
                self._area = Length(self._area.value**2, self._area.unit, 2, "cm")

    # calculated properties
    @property
    def volt(self):
        """V = E * t."""
        return Volt(self.efield.V * self.thick.cm)
    ...
    def resistance(self):
        """R = rho * t / A."""
        return Res(self.resistivity * self.thick.cm / self.area.cm)

    @resistance.setter
    def resistance(self, val):
        """Return sum of squared errors (pred vs actual)."""
        if val is not None:
            resis = self.parse_unit(Res, val, "Ohm")
            self.resistivity = resis.Ohm * self.area.cm / self.thick.cm

    @property
    def curr(self):
        """Return sum of squared errors (pred vs actual)."""
        return Curr(self.volt.V / self.resistance.Ohm)
    ...
    def cap(self):
        """Return sum of squared errors (pred vs actual)."""
        return self.er * Length(PERM, "m", -1).cm * self.area.cm / self.thick.cm

    @cap.setter
    def cap(self, val):
        """Return sum of squared errors (pred vs actual)."""
        if val is not None:
            self.er = val * self.thick.cm / (self.area.cm * Length(PERM, "m", -1).cm)

    @property
    def charge(self):
        """Return sum of squared errors (pred vs actual)."""
        return self.er * Length(PERM, "m", -1).cm * self.resistivity * self.curr.A

    @charge.setter
    def charge(self, val):
        """Return sum of squared errors (pred vs actual)."""
        if val is not None:
            self.er = val / (Length(PERM, "m", -1).cm * self.resistivity * self.curr.A)
    ...
    def source(self):
        ...
        if not hasattr(self, "_source"):
            self._source = 1e21
        if self.cp_link:
            return poisson(self.er, volt=self.volt.V, thick=self.thick.cm) / 10
        else:
            return self._source
    ...
    def np_solver(
        self, target, source=None, conc=None, scale="lin", ls_params={}, **pre_kwargs
    ):
        """Return sum of squared errors (pred vs actual)."""
        if target in self.info_np.columns and self._dif != 0:
            ...
            if "np" in self.np_type or target == "temp":
                self[target] = nernst.np_sim(
                    target, ratio, scale, ls_params, **pre_kwargs
                )
            else:
    ...
    def parse_arrh(self, param, *args):
        ...
        if isinstance(args[0], BaseUnits):
...
class Stack:
    ...
    def __init__(
        ...
        **layer_kwargs,
        ...
        if isinstance(layers, (int, np.integer)):
            ...
            self.layers = [
                Layer(material=mat, data_guide=guide, **layer_kwargs)
                for mat, guide in layers
            ]
    ...
    def __getitem__(self, item):
        ...
        if isinstance(items[0], BaseUnits):
    ...
    def stack_info(self):
        """Return sum of squared errors (pred vs actual)."""
        df = pd.concat(
            [lay.info_electrical for lay in self.layers], axis=0, ignore_index=True
        )
        return df
...
class VoltDev(Stack):
    ...
    def sys_time(self):
        """Calculate. generic discription."""
        # return Res(self.stack.resistance.Ohm.sum())
        return Time(self["time"].s.sum())

    @property
    def sys_res(self):
        """Calculate. generic discription."""
        # return Res(self.stack.resistance.Ohm.sum())
        return Res(self["resistance"].Ohm.sum())

    @property
    def sys_volt(self):
        """Calculate. generic discription."""
        if not hasattr(self, "_sys_volt"):
            self._sys_volt = Volt(1)
        return self._sys_volt

    @sys_volt.setter
    def sys_volt(self, val):
        if isinstance(val, Volt):
            self._sys_volt = val
        elif isinstance(val, (tuple, list)):
            self._sys_volt = Volt(*val)
        elif isinstance(val, (dict)):
            self._sys_volt = Volt(**val)
        else:
            self._sys_volt = Volt(val, "V")
    ...
    def curr_eval(self, current=0, lay_params={}, alter_volt=False):
        ...
        if isinstance(current, Curr):
            tar_curr = current
        else:
            tar_curr = Curr(current)
        if lay_params == {}:
        ...
        eff_volt = Volt(self.sys_volt.V - added_layer.volt.V)
    ...
    def cap_eval(
        ...
        while dev > qtol and n < iters:
            ...
            eps1 = self.sys_charge / (
                Length(PERM, "m", -1).cm * self["efield"].V * self["area"].cm
            )
            eps_dev = eps0 - eps1
...
class Module(object):
    ...
    def result(self, layer=None, param=None, unit=None, as_df=False):
        ...
        if as_df:
            df = self.conv_to_num(
                pd.DataFrame(self.stack.sys_units.items(), columns=["param", "unit"]),
                df,
            )
